...
# Import modules
from Dataset import dataset_reference_deformable
...
class DataModule: 
    ...
    def _dataloader(self, dataloader_type):
        ...
        return DataLoader(
            dataset=self.datasets[dataloader_type], 
    ...
    def _dataset(self, csv_loc, key):
        ...
        if self.config["augmentation"]["apply"]:
            return dataset_reference_deformable(csv_loc, self.config["training"]["hist_eq"], self.config["training"]["include_lowres"], key=key, affine_trans = self.config["augmentation"]["affine"]["prob"], smooth=self.config["augmentation"]["smooth"]["prob"], transform=self.compose_transformation()) 
        else:   
            print("Im in else")                                                                                                               
            return dataset_reference_deformable(csv_loc, self.config["training"]["hist_eq"], self.config["training"]["include_lowres"], key=key, affine_trans = 0, smooth=self.config["augmentation"]["smooth"]["prob"], transform=None)
