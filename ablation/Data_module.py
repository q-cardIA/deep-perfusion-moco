...
class DataModule: 
    ...
    def _dataset(self, csv_loc, key):
        ...
        if self.config["augmentation"]["apply"]:
            return dataset_reference_affine(csv_loc, self.config["training"]["hist_eq"], self.config["training"]["include_lowres"], key=key, transform=self.compose_transformation())
        else:
            return dataset_reference_affine(csv_loc, self.config["training"]["hist_eq"], self.config["training"]["include_lowres"], key=key, cut_edge=self.config["training"]["cut_side"], transform=None)
