    ...
    def early_stop(self, validation_loss):
        if validation_loss < self.min_validation_loss:
            ...
            if self.counter >= self.patience:
                return True
        return False
def loss_function(config, fixed_image, predicted_image, fixed_train_mri, pred_image_training_mri, ddf, mode):
    """
    ...
    # Loss between MRI and RPCA images 
    if config["loss"]["mri_rpca_loss"]["MI"] > 0: # and mode == "train": 
        gmi_loss = GlobalMutualInformationLoss()
        rpca_mri_loss = gmi_loss(pred_image_training_mri, fixed_image) + gmi_loss(predicted_image, fixed_train_mri)
    elif config["loss"]["mri_rpca_loss"]["SSIM"] > 0: # and mode == "train":
        ssim_loss_func = ssim_loss.SSIMLoss(spatial_dims=2)
        rpca_mri_loss = ssim_loss_func(pred_image_training_mri, fixed_image) + ssim_loss_func(predicted_image, fixed_train_mri)
    elif config["loss"]["mri_rpca_loss"]["NCC"] > 0: # and mode == "train":
        ncc_loss_func = LocalNormalizedCrossCorrelationLoss(spatial_dims=2, kernel_size=127)
        rpca_mri_loss = ncc_loss_func(pred_image_training_mri, fixed_image) + ncc_loss_func(predicted_image, fixed_train_mri)
    else:
        rpca_mri_loss = 0
    ...
    return lam_ncc*ncc + lam_gmi*gmi + lam_reg*reg + rpca_mri_loss, ncc, rpca_mri_loss, reg

def save_images(type_training, epoch, fixed, moving, pred_image, pred_image_mri,  ddf, warp_layer, save_model_path):
    ...
    nr_rows = 8
    fig, ax = plt.subplots(nr_rows, 5, figsize=(12, 3 * nr_rows))
    for ind_mon in range(nr_rows):
        ...
        ax[i, 3].set_title("Predicted image mri")
        ax[i, 3].axis("off")
        if type_training == "train":
            ax[i, 3].imshow(pred_image_mri[ind_mon][0], cmap="gray")
        else:
            ax[i, 3].imshow(pred_image_mri[ind_mon], cmap="gray")
        
        ax[i, 4].set_title("Deformation")
        ax[i, 4].axis("off")
        ax[i, 4].imshow(image_pred_mon.detach().cpu().numpy()[ind_mon,0,:,:])
    plt.tight_layout()
...
def main(config, save_model_path, loss_func="LNCC"):
    ...
    monitor_metrics = {"training": {"LNCC":[], "GMI":[], "SSIM":[], "reg":[], "mi_rpca":[]},
                       "validation": {"LNCC":[], "GMI":[], "SSIM":[], "reg":[], "mi_rpca":[]}}
    ...
    for epoch in range(max_epochs):
        ...
        mi_rpca_val, mi_rpca_val_valid =0,0 # monitor mi between rpca and mri images
        ...
        for batch_data in train_loader:
            ...
            moving_train_mri = batch_data["image_mri"].to(torch.float).to(device).unsqueeze(1)     # mri
            fixed_train_mri = batch_data["reference_mri"].to(torch.float).to(device).unsqueeze(1)  # mri
            moving_train_org = batch_data["image"].to(torch.float).to(device).unsqueeze(1)         # rpca (either hist eq. or not)
            fixed_train_org = batch_data["reference"].to(torch.float).to(device) .unsqueeze(1)     # rpca (either hist eq. or not)

            ddf = model(torch.cat((moving_train_mri, fixed_train_mri), dim=1))              # get deformation field from model
            pred_image_training = warp_layer(moving_train_org, ddf)                         # apply deformation field to moving image
            pred_image_training_mri = warp_layer(moving_train_mri, ddf)                     # apply deformation field to moving image (mri)

            if step == 10:
                save_images("train", epoch, fixed_train_mri.detach().cpu().numpy()[:8], moving_train_mri.detach().cpu().numpy()[:8], pred_image_training.detach().cpu().numpy()[:8], pred_image_training_mri.detach().cpu().numpy()[:8] ,ddf, warp_layer, save_model_path)

            # Calculate the multi-element loss 
            loss, ncc, mi_rpca, reg_train = loss_function(config, fixed_train_org, pred_image_training, fixed_train_mri, pred_image_training_mri, ddf, "train") # calculate loss beween fixed and moved
            ...
            ssim_train_val += metrics["SSIM"](pred_image_training, fixed_train_org).item()
            if ncc != 0:
                lncc_train_val += ncc.item()
            else:
                lncc_train_val += loss.item()
            reg_train_val += reg_train
            mi_rpca_val += mi_rpca.item()
        ...
        monitor_metrics["training"]["mi_rpca"].append(mi_rpca_val/step)
        ...
        with torch.no_grad():  
            ...
            for batch_data in valid_loader:
                ...
                hist_eq = True
                moving_valid_mri = batch_data["image_mri"].to(torch.float).to(device)           # mri
                fixed_valid_mri = batch_data["reference_mri"].to(torch.float).to(device)        # mri
                moving_valid_org_hist = batch_data["image"].to(torch.float).to(device)  	    # hist_eq(rpca)
                fixed_valid_org_hist = batch_data["reference"].to(torch.float).to(device)       # hist_eq(rpca)
                
                moving_valid_mri = moving_valid_mri.unsqueeze(1)
                fixed_valid_mri = fixed_valid_mri.unsqueeze(1)
                moving_valid_org_hist = moving_valid_org_hist.unsqueeze(1)
                fixed_valid_org_hist = fixed_valid_org_hist.unsqueeze(1)

                if hist_eq == True:
                    moving_valid_org = moving_valid_org_hist
                    fixed_valid_org = fixed_valid_org_hist

                ddf_valid = model(torch.cat((moving_valid_mri, fixed_valid_mri), dim=1))      
                pred_image_valid = warp_layer(moving_valid_org, ddf_valid)
                pred_image_valid_mri = warp_layer(moving_valid_mri, ddf_valid)
                ...
                moving_monitor.append(moving_valid_org.detach().cpu().numpy()[0,0])
                fixed_monitor.append(fixed_valid_org.detach().cpu().numpy()[0,0])
                moved_monitor.append(pred_image_valid.detach().cpu().numpy()[0,0])
                square_monitor.append(image_pred_mon.detach().cpu().numpy()[0,0])
            
                # Calculate the multi-element loss 
                valid_loss, NCC_valid, mi_rpca_valid, reg_valid = loss_function(config, fixed_valid_org, pred_image_valid, fixed_valid_mri, pred_image_valid_mri, ddf_valid, "valid")
                valid_epoch_loss += valid_loss
                
                # Monitor LNCC/GMI/SSIM during validation
                ssim_valid_val += metrics["SSIM"](pred_image_valid, fixed_valid_org).item()
                lncc_valid_val += NCC_valid
                reg_valid_val += reg_valid
                mi_rpca_val_valid += mi_rpca_valid
        ...
        monitor_metrics["validation"]["mi_rpca"].append(mi_rpca_val_valid/step_valid)
        print(f"epoch {epoch + 1} average validation loss: {valid_epoch_loss:.4f}")

        # Save best model
        if epoch > 30 and best_loss > valid_epoch_loss.detach().cpu().numpy():
...
if __name__ == "__main__":
    ...
    print("Working on config_{}".format(test_number))
    ...
    extra_info = "optimmized_with_{}".format("NCC")
    print("Loss")
    print(epoch_loss_values)
    print(epoch_loss_values_validation)
    print()
    print("Metric")   
