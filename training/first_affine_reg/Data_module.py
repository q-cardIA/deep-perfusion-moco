...
class DataModule: 
    ...
    def _dataset(self, csv_loc, key):
        ...
        if self.config["augmentation"]["apply"]:
            return dataset_reference_affine(csv_loc, self.config, key=key, transform=self.compose_transformation())

        else:
            return dataset_reference_affine(csv_loc, self.config, key=key, transform=None)
