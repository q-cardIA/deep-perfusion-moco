...
from monai.losses import ssim_loss, BendingEnergyLoss, LocalNormalizedCrossCorrelationLoss, GlobalMutualInformationLoss
...
def main(config, save_model_path, loss_func="LNCC"):
    ...
    for epoch in range(max_epochs):
        ...
        for batch_data in train_loader:
            ...
            moving_training = batch_data["image"].to(torch.float).to(device)
            fixed_training = batch_data["reference"].to(torch.float).to(device)

            moving_training = moving_training.unsqueeze(1)
            fixed_training = fixed_training.unsqueeze(1)
        ...
        with torch.no_grad():  
            ...
            for batch_data in valid_loader:
                ...
                moving_valid = batch_data["image"].detach().to(torch.float).to(device)
                fixed_valid = batch_data["reference"].detach().to(torch.float).to(device)
                
                moving_valid = moving_valid.unsqueeze(1)
                fixed_valid = fixed_valid.unsqueeze(1)
...
if __name__ == "__main__":
    ...
    plt.plot(range(config["training"]["num_epochs"]), epoch_loss_values, label="train")
    plt.plot(range(config["training"]["num_epochs"]), epoch_loss_values_validation, label="valid")
    plt.legend()
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Loss curve {}".format(extra_info))
    plt.savefig("{}loss_curve.png".format(save_image_path))
