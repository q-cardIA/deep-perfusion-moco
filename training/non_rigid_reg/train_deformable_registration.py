...
def loss_function(config, fixed_image, predicted_image, ddf):
    ...
    if lam_ncc > 0:
        ncc_loss = LocalNormalizedCrossCorrelationLoss(spatial_dims=2, kernel_size=config["loss"]["LNCC"]["kernel_size"])
    if lam_gmi > 0:
...
def save_images(type_training, epoch, fixed, moving, pred_image, ddf, warp_layer, save_model_path):
    ...
    array_mon = np.zeros((config["dataloader"][type_training]["batch_size"],1,96, 96))
    array_size_mon, square_size_mon = 96, 40
    for ind_mon in range(config["dataloader"][type_training]["batch_size"]):
        image_mon = np.zeros((96, 96))
...
def main(config, save_model_path, loss_func="LNCC"):
    ...
    model = LocalNet(spatial_dims=2, in_channels=2, out_channels=2, num_channel_initial=config["model"]["num_channel_initial"], extract_levels=config["model"]["extract_levels"]).to(device)

    # Define loss function
    metrics = {"GMI" :GlobalMutualInformationLoss(),
               "SSIM":ssim_loss.SSIMLoss(spatial_dims=2)}
    monitor_metrics = {"training": {"loss":[], "LNCC":[], "GMI":[], "SSIM":[], "reg":[]},
                       "validation": {"loss":[], "LNCC":[], "GMI":[], "SSIM":[], "reg":[]}}

    # Define warp layer and optimizer
    ...
    epoch_loss_values, epoch_loss_values_validation = [], []
    best_loss = 1000000
    for epoch in range(max_epochs):
        ...
        for batch_data in train_loader:
            ...
            # Get deformation field and apply to image
            ddf = model(torch.cat((moving_training, fixed_training), dim=1))
            pred_image_training = warp_layer(moving_training, ddf) 
        ...
        monitor_metrics["training"]["loss"].append(epoch_loss)
        ...
        with torch.no_grad():  
            ...
            for batch_data in valid_loader:
                ...
                array_mon = np.zeros((config["dataloader"]["valid"]["batch_size"],1,96, 96))
                array_size_mon, square_size_mon = 96, 40
        ...
        monitor_metrics["validation"]["loss"].append(valid_epoch_loss.item())
        ...
        if epoch > 30 and best_loss > valid_epoch_loss.detach().cpu().numpy():
            torch_input = torch.randn(1,2,96,96).to(device)
