...
class dataset_reference_affine(Dataset):
    def __init__(self, csv_file, config, key, transform=None):
        """
        Arguments:
        ...
        # Not used yet
        self.lowres_data = {}
        self.include_lowres = config["training"]["include_lowres"]
        ...
        self.apply_hist_eq = config["training"]["hist_eq"]
        self.hist_eq_mri = config["training"]["hist_eq_mri"]
    ...
    def __getitem__(self, idx):
        ...
        # Load moving and fixed image (low-rank output of RPCA)
        ...
        if self.hist_eq_mri:
            # Histogram equalization
            image_arr_mri = equalize_hist(image_arr_mri)
            reference_arr_mri = equalize_hist(reference_arr_mri)

        # Apply augmentations
        if self.transform != None and self.key != "valid": 
            # Convert shape of image to (bs, nr_channel, x, y)
            
            # Get (random) affine transformation, to apply to both moving images
            ...
            # Augment MR images
            img_new = np.empty((1,128,128))
            img_new[0,:,:] = image_arr_mri
            image_arr_mri = transform_affine(img_new)[0][0]
                # also apply intensity augmentations
            img_new = np.empty((1,128,128))
            img_new[0,:,:] = image_arr_mri
            image_arr_mri = self.transform(img_new)[0]

            # Augment original images
            ref_new = np.empty((1,128,128))
            ref_new[0,:,:] = image_arr_org
            image_arr_org = transform_affine(ref_new)[0][0]
            ...
            # Augment MRI images
            ref_new = np.empty((1,128,128))
            ref_new[0,:,:] = reference_arr_mri
            reference_arr_mri = transform_affine(ref_new)[0][0]
                # also apply intensity augmentations
            img_new = np.empty((1,128,128))
            img_new[0,:,:] = reference_arr_mri
            reference_arr_mri = self.transform(img_new)[0]

            # Augment original images
            ref_new = np.empty((1,128,128))
            ref_new[0,:,:] = reference_arr_org
            reference_arr_org = transform_affine(ref_new)[0][0]

        sample = {'image_mri':image_arr_mri, 'reference_mri':reference_arr_mri, 'image':image_arr, 'reference':reference_arr, 'image_org':image_arr_org, 'reference_org':reference_arr_org}
